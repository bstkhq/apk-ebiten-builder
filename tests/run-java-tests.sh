...
perl -pe 's/\@\@APP_ID\@\@/test.builder/g' \
  "${repo_dir}/android/app/src/main/java/OptionalMobileHooks.java" \
  > "${source_dir}/OptionalMobileHooks.java"
cp "${repo_dir}/tests/java/OptionalMobileHooksTest.java" "${source_dir}/"

javac -Xlint:all -Werror -d "${classes_dir}" "${source_dir}"/*.java
java -ea -cp "${classes_dir}" test.builder.OptionalMobileHooksTest
