...
for source in OptionalAndroidBridge OptionalBackBridge OptionalFilePickerBridge; do
  perl -pe 's/\@\@APP_ID\@\@/test.builder/g' \
    "${repo_dir}/android/app/src/main/java/${source}.java" \
    > "${source_dir}/${source}.java"
done
cp "${repo_dir}/tests/java/OptionalAndroidBridgeTest.java" \
  "${repo_dir}/tests/java/OptionalBackBridgeTest.java" \
  "${repo_dir}/tests/java/OptionalFilePickerBridgeTest.java" \
  "${source_dir}/"
...
java -ea -cp "${classes_dir}" test.builder.OptionalFilePickerBridgeTest
